<!DOCTYPE html>
<html lant="es">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Primer Script PHP</title>
    </head>
    <body>
        <h1>Primer Script en PHP</h1>
        
<?php
// Script: 00primer_script.php
echo "Esto es un script en PHP<br>";  // La sentencia echo emite una expresión a la respuesta
echo "Esto lo ejecuta el motor de PHP<br>";

# Sentencia compuesta
{
    echo "Primer sentencia en un bloque de sentencias<br>";  # La sentencia echo emite una expresión a la respuesta
    echo "Segunda sentencia en un bloque de sentencias<br>";
}

/*
Esto es un comentario multilínea
que ocupa varias líneas
para poner explicaciones
más largas.
*/
?>
        <br>
        <p>Fin del script PHP</p>
    </body>
</html>